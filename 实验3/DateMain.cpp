...
{
    private:
        int price;char place[10],material[10];//单价，产地，材质
    public:
        Base(int pri,char *pla,char *mat)//构造
        {
            price=pri;
            strcpy(place,pla);
            strcpy(material,mat);
...
class shirt:public Base//派生shirt类
{
    public:
...
class cap:public Base//派生cap类
{
    private:
        char style[10];
    public:
        cap(int pri,char *pla,char *mat,char *sty):Base(pri,pla,mat)//增加样式
        {
            strcpy(style,sty);
        }
        int GetPrice(){return Base::GetPrice();}
...
class Wardrobe:public Base//增加立柜
{
    private:
        char color[10];
    public:
        Wardrobe(int pri,char *pla,char *mat,char *col):Base(pri,pla,mat)//增加颜色
        {
            strcpy(color,col);
        }
        int GetPrice(){return Base::GetPrice();}
